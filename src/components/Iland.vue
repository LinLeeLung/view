<template>
    <div class="bg-white p-4 rounded-lg shadow-md min-w-[450px]">
      <div class="flex items-center mb-2 space-x-2">
        ...
        />
        <h2 class="text-md font-semibold text-gray-700">中島計價</h2>
      ...
      <table class="w-full table-fixed">
        <thead>
          <tr class="text-sm text-left text-gray-600">
            ...
            <th class="w-1/6">左倒包</th>
            <th class="w-1/6">右倒包</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td><input v-model.number="form.length" type="number" min="0" class="w-10 p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
            <td><input v-model.number="form.depth" type="number" min="0" class="w-10 p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
            <td><input v-model.number="form.frontEdge" type="number" min="0" class="w-8 p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
            <td><input v-model.number="form.backEdge" type="number" min="0" class="w-8 p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
            ...
            <td><input v-model.number="form.wrapLeft" type="number" min="0" class="w-full p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
            <td><input v-model.number="form.wrapRight" type="number" min="0" class="w-full p-1 border rounded-md text-sm" :disabled="!isEnabled" /></td>
          </tr>
        </tbody>
      </table>
